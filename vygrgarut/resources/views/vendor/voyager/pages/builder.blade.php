...
@section('page_title', 'Page Builder ' . __($data[0]->title))

@section('page_header')
<h1 class="page-title">
    <i class="voyager-puzzle"></i>{{ __('Component Builder') }} ({{ $data[0]->title }})
    <a href="{{ route('component.create', $data[0]->id) }}" class="btn btn-success btn-add-new">
        ...
        <div class="col-md-12">
            <div class="panel panel-bordered">
                <div class="panel-heading">
                    <p class="panel-title" style="color:#777">{{ __('component.drag_drop_info') }}</p>
                </div>
                        ...
                        <ol class="dd-list">
                            ...
                            <li class="dd-item" data-id="{{ $item->id }}">
                                <div class="pull-right item_actions">
                                    ...
                                    <a href="{{ route('component.read', ['page' => $data[0]->id, 'id' => $item->id]) }}"
                                    ...
                                    <a href="{{ route('component.edit', ['page' => $data[0]->id, 'id' => $item->id]) }}"
                                    ...
                                    <a href="{{ route('item.builder', ['page' => $data[0]->id, 'id' => $item->id]) }}"
                                        class="btn btn-sm btn-success pull-right" style="margin-right: 5px;">
                                        <i class="voyager-list"></i> <span
                                            class="hidden-xs hidden-sm">{{ __('voyager::generic.builder') }}</span>
                                    </a>
                                </div>
                                <div class="dd-handle">
                                    <span>{{ $item->title }}</span> <small
                                        class="url">{{ $item->type->name }}</small>
                                    <small class="url">@if($item->status == 'hidden'){{ 'hidden' }}@else{{ 'show' }}@endif</small>
                                </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><i class="voyager-trash"></i>
                    {{ __('voyager::component.delete_question') }}</h4>
            </div>
            <div class="modal-footer">
                <form action="{{ route('component.destroy', ['id' => $item->id]) }}" id="delete_form"
                    method="POST">
                    {{ method_field("DELETE") }}
                    {{ csrf_field() }}
                    <input type="submit" class="btn btn-danger pull-right delete-confirm"
                        value="{{ __('component.delete_confirm') }}">
                </form>
...
<script>
        ...
        */
        @if ($data[0]->components != '[]')
            $('.item_actions').on('click', '.delete', function (e) {
                ...
                $('#delete_form')[0].action = '{{ route('component.destroy', ['id' => $item->id, 'id' => '__id']) }}'.replace('__id', id);
    ...
    */
    $('.dd').on('change', function (e) {
        $.post('{{ route('components.order_item',['id' => $data[0]->id]) }}', {
